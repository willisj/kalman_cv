...
# coding: latin-1
'''
# <codecell>    pykal1.py    Kalman Filter for GPS trail (path trace) smoothing
#
#    Ian Douglas    2015-August
'''
...
def positions_from_offsets_corrective(refs,offsets):
    ...
    if not (len(refs) == len(offsets)):
        print "ERROR: positions from offsets not matching in length. %d  vs.  %d" % (len(refs), len(offsets))
        exit(1)
...
def run_kal(measurements, training_size=40):
    ...
    if dim == 3:
        ...
        exit(1)
    print("run_kal smooth --> simple_mode={0}".format(simple_mode))
...
def load_gps_trail(extra_fields = False):
    ...
    measurements = []
    header=True
    print("Failure here means the input gps file did not have correct Unix LF endings...")
    for line in open('nazeer_trip_400-2100-LF.csv'):        #'nazeer_trip.csv'):
        if header:
            header=False
            continue
        i += 1
        ...
        if i % 6 == 0:
            ...
            print i,fields, extra_fields #, measurements
            continue
...
def plot_all_dimensions(positions,file_prefix):
    ...
    plot_first_dim(y_vals,flip_xy=False,xlbl="Time (s)",ylbl="UTM Longitude",title="UTM LONGITUDE OVER TIME",filename=output_dir +file_prefix + "all_dim_y")
...
print "Main Line 271 - measurements ", measurements

# Convert these measurements to meters
cart_measurements = gps_to_cartesian(measurements)
print "Main Line 275 - cart_measurements ", cart_measurements
...
print "Run KF - cart_measurements", cart_measurements
ss_means_meas, ss_covariance_meas,cart_simple = run_kal(cart_measurements,training_size=40)
filtered_cart_measurements = ss_means_meas        # Give this a more meaningful name
print "Run KF - filtered_cart_measurements", filtered_cart_measurements
...
'''
##    GPS Trail input and Kalman filter smoothing...
##
##  End <pykal2.py>
'''